...
export async function POST(request: Request) {
    try {
        ...
        if (!["admin", "manager", "cashier"].includes(payload.role)) {
        ...
        const result = await db.transaction(async (tx) => {
            ...
            for (const item of items) {
                ...
                // Resolve target location (body locationId or first active location)
                let targetLocation
                if (locationId) {
                ...
                // Keep backward-compatible product stock in sync (decrement in base units)
                await tx
                    .update(products)
                    .set({ stock: sql`${products.stock} - ${stockQty}` })
                    .where(eq(products.id, item.productId))

                // Update per-location stock in base units
                if (targetLocation) {
